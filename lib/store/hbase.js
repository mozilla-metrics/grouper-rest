...
var exports = module.exports = function(conf, next, stack, cb) {
  ...
  , getAllClusters: function (q, cb) {
      // :TODO:
      // Instead of glob-get, we might use a scanner and emit our
      // response in a streaming fashion..., something like:
      /*
      hbase.getTable(CLUSTERS).getScanner({
        ...
        , filter: {
          ...
          , op: 'EQUAL'
          , comparator: {
              value: keyFor.clusters(q)
            , type: 'BinaryPrefixComparator'
            }
          }
        }
      }).get(function (err, cells) {
        // ....rewrap the scanner response:
