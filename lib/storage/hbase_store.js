...
function HBaseStore(conf, next, stack) {
  ...
  var DOCUMENTS = conf.tableName('documents');
  ...
  this.putDocument = function putDocument_(document, call) {
    ...
    // TODO:
    // Ideally we would update the collection size here, but hbase REST does
    // not seem to expose atomic increment. So we will update it during
  ...
  this.getCollection = function getCollection_(collectionRef, call) {
    ...
    // The handling of collections is just extremely awful right now...
    // TODO: have a different model object for clustering configurations.
    // TODO: Get rid of JSON-storage and most of the explicit "DEFAULT"s
    //       There is *one* place for DEFAULT: the app config...

    var configuration;
    var size = {};
    var attributesPrefix ="main:configurations:";
    var prefixLength = 'main:configurations:'.length;
    function receive(err, cells) {
      if (err) return call.cb(err);
      cells.forEach(function (cell) {
        if (cell.column == "main:size") {
          size = cell.$;
          return;
        }
        if (startsWith(cell.column, attributesPrefix)) {
          var name = cell.column.substr(prefixLength);
          if (name != "DEFAULT") return;
          configuration = JSON.parse(cell.$);
        }
      });
      var result = new Collection(collectionRef,
                                  {SIZE: size,
                                   REBUILT: configuration.lastRebuild});
      call.cb(null, result);
    }
  ...
  this.toString = function toString_() {
    return ['(HBaseStore in ', conf.prefix, '*)'].join('');
  };
...
function HBaseAdmin(conf) {
  ...
  function dropTable(name, cb) {
    ...
    var table = client.getTable(conf.tableName(name));
